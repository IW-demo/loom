...
though `terrene-foundation/loom` (own host) on the same surface does not.
...
A different operator home: /Users/notexample/repos/loom is still a finding.

Own-coordinates appearing alongside MUST NOT mask the above:
terrene-foundation/loom is the own host (does not flag);
/Users/example/repos/loom is the own dev path (does not flag).
