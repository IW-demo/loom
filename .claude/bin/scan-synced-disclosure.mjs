...
const ALLOWLIST = [
  ...
  /terrene\.dev/i,
  // ALLOWLIST-NOTE: `terrene-foundation` is loom's own GitHub host org
  ...
  // and `terrene-foundation/<repo>` self-references (the same shape as
  // the Foundation entry above). R2 SECURITY-FIX (#263): the prior
  // entry's stem was UNANCHORED — `terrene-foundation-evil/loom` and
  // `gh api repos/terrene-foundation-evil/kailash-py` were SUPPRESSED
  // (a typosquat org leaked silently). The trailing `(?![\w-])`
  // non-word/non-hyphen boundary anchors the allowlist to the EXACT
  // own org: `terrene-foundation` and `terrene-foundation/<repo>`
  // stay clean; `terrene-foundation-evil/loom` no longer matches the
  // allowlist and is flagged by the nonfoundation-org-slug shape.
  /terrene-foundation(?![\w-])(\/[A-Za-z0-9._-]+)?/i,
  // ALLOWLIST-NOTE: loom maintainer's own dev-home-path prefixes
  // (`/Users/<user>/` and `/home/<user>/`) are self-coordinates under
  ...
  //   /home/<user>/, /Users/<user>/ — the canonical "me" placeholder in CC/Codex
  ...
  /\/Users\/runner\//,
  /\/(?:Users|home)\/me\//,
  // ALLOWLIST-NOTE: a `/Users/<PascalCase>/` span (e.g. `/Users/<user>/`
  // from the `mockData/Users/<user>/Records/Response*` glob comment in
...
const SHAPES = [
  ...
  },
  {
    ...
    // The leading `(?!…terrene-foundation|terrene-foundation…)`
    ...
    // (`terrene-foundation/loom`, `terrene-foundation/loom`) DO match
    ...
    // INCLUDING any trailing `-<suffix>` segments (`terrene-foundation-evil`,
  ...
  },
  {
    ...
    // `/Users/<user>/` is wrong INSIDE a `permissions.*` matcher in a
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const shape of shapes) {
      ...
      while ((m = shape.rx.exec(line)) !== null) {
        ...
        // this shape so own-coordinate `/Users/<user>/` tokens inside
