...
Then emit the unified `.coc/` derivative (#392): `node .claude/bin/emit-coc.mjs --target <variant> --out .`. Unlike `emit-cli-artifacts.mjs`, this writes the dotted target `.coc/` directly via an internal atomic tmp-dir swap — invoke with `--out .` (NOT a tmp dir + move). It produces `COC.md` + `COC.lock` + `rules/agents/skills/commands/` conforming to the csq consumer contract (`governance.csq:specs/09-unified-coc-artifact-standard.md`). A `WARN: … > 60 KiB` line is advisory (spec-09 has no consumer cap), not a failure.
