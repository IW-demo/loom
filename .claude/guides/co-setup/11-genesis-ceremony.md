...
3. **No `refs/coc/**` ruleset equivalent — MUST-5 degrades to client-side detection-only on ADO.** GitHub's server-side ref-protection ruleset (`rules/multi-operator-coordination.md`MUST-5) is the prevention-primary equivocation-parity defense; ADO has no equivalent ref-namespace ruleset. On ADO, MUST-5 degrades to the provider-neutral client-side detection layer (F51 archive-tip-pin verification + fold-rule equivocation detection). This is a documented degraded residual — see`rules/multi-operator-coordination.md` MUST-5 ADO clause + the forest item in that rule's § Origin registry.
