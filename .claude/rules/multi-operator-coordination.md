...
**Azure DevOps degraded residual (F122).** The `refs/coc/**` ruleset is a GitHub-specific server-side primitive; Azure DevOps exposes NO equivalent ref-namespace ruleset (ADO branch policies do not cover arbitrary custom ref namespaces the way GitHub rulesets cover `refs/coc/**`). On a repo with `roster.genesis.provider: "azure-devops"`, MUST-5's prevention-primary server layer is therefore UNAVAILABLE, and the equivocation-parity defense degrades to the provider-neutral CLIENT-SIDE detection layer alone: the F51 archive-tip-pin verification (`fold-rule-9b.js` + `archive-ref.js::verifyArchiveTipPin`, both provider-neutral — they operate on the pinned tip hash, not on any GitHub API) plus fold-rule equivocation detection (fold rule 3 fork-detection, provider-neutral). This is a DOCUMENTED, ACCEPTED degraded residual — detection-eventually, not prevention-primary — surfaced honestly per the bounded-trust threat model's §1.1 general structural-residual law. An ADO repo MUST NOT claim the prevention-primary posture; the forest item in § Origin tracks the open question of an ADO-native server-side equivalent (e.g. an ADO branch-policy or REST guard) should one become available. Provisioning the GitHub `refs/coc/**` ruleset remains MANDATORY for GitHub-provider repos — the degraded path applies ONLY to the ADO provider.
...
{ "type": "genesis-migration", "content": { "co_signers": [ { "person_id": "pid-example-.." } ] } }  # BLOCKED R6-S-04
```
...
- **F122-ADO-REF (OPEN) — ADO-native server-side ref-protection equivalent for MUST-5.** Azure DevOps repos run MUST-5 in the degraded client-side-detection-only posture (no `refs/coc/**` ruleset equivalent — see the MUST-5 ADO clause). Acceptance criteria for closure: (1) determine whether ADO exposes ANY server-side primitive that can restrict creation/deletion of a custom ref namespace (candidate surfaces to existence-check per `verify-resource-existence.md` MUST-2: ADO branch policies, the `policy/configurations` REST API, repository-level security namespaces / ACLs on `refs/coc/*`, or a service-hook guard) — cite the LIVE API result, not docs; (2) if a primitive exists, wire it as the ADO analogue of the GitHub ruleset (provisioning command + a `vcs-azure-adapter.js` deploy helper + the seed-first ordering equivalent) and upgrade the ADO MUST-5 posture from detection-only to prevention-primary; (3) if NO primitive exists, record the negative existence result here as the durable receipt and keep the degraded residual documented (the bounded-trust threat model already accepts detection-eventually). Call-graph anchor: `vcs-azure-adapter.js` (the deploy helper would live here, sibling to `fetchRepoOwner`/`fetchOrgAdmin`); the MUST-5 ADO clause above is the prose receipt. Origin: F122 Shard 4 (ADO prose/runbook) — the ADO degraded-residual surfaced honestly during the Azure DevOps ceremony port; user-approved design decision #2 (ADO ref-protection = documented degraded residual now).
