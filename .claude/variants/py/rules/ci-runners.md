...
> (`<runner-host-1>`, `<runner-host-2>`, `<runner-label-arm>`, `<runner-name>`).
> Operator-specific concrete values for THIS deployment live in
> `.claude/variants/py/rules/ci-runners.operator.local.md` (gitignored, never
> synced — issue #260). Its schema and how to populate it are documented in the
> committed `.claude/variants/py/rules/ci-runners.operator.local.example.md`.
