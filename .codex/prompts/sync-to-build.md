...
Read `sync-manifest.yaml` → `repos.{target}.build` to get the BUILD repo logical NAME. Resolve its on-disk path via `bin/lib/loom-links.mjs::resolveRepo("build.{target}")` — the canonical NAME→location binding (`cross-repo.md` MUST-1), never a positional `../{build}` / `~/repos/{build}` guess.
...
Otherwise resolve the absolute path via `resolveRepo("build.{target}")` (an undeclared linkage is a typed `LinkError`, surfaced — not a positional fallback) and proceed to step 2.

### 2. Inventory the BUILD repo (READ FIRST)

Before computing what to push, read what the BUILD repo already has:

1. List ALL directories under BUILD `.codex/skills/` — note any that use different numbering than loom/ source
...
- `.claude/bin/*.local.json` — gitignored operator-local config (loom-links
  registry; mirrors `/sync`'s `LOOM_LOCAL_PATTERNS` at
  `.claude/bin/sync-tier-aware.mjs::LOOM_LOCAL_PATTERNS`). Parity per #352.
  The companion `*.local.example.json` schema templates DO ship as committed
  schemas; only the operator-local `.local.json` instances stay out.
- `CLAUDE.md` — repo-specific (never overwritten)

The `.claude/bin/*.local.json` exclusion mirrors `/sync`'s loom-local exclusion; both outbound paths should converge on a shared `isNeverSynced` predicate so the two never drift (Stage-2 follow-up under #350's umbrella scaffold).

**Enforcement note:** before any write in step 6, the `coc-sync` delegate MUST read this exclusion list AND `sync-manifest.yaml::exclude:` and reject any candidate path matching either. Mechanical enforcement lives in the `coc-sync` agent's `/sync-to-build` step (and `sync-tier-aware.mjs` for the /sync path). Skipping the read is BLOCKED per `rules/zero-tolerance.md` Rule 3 (silent fallback).
